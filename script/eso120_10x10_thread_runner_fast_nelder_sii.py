...
L_SII = tc.lines.Line(6730.81, plus=11, minus=40, label="SII")
...
def SIImodparams():
    ...
    model.set_param_hint("g1_center", expr="g2_center+deltax")
    model.set_param_hint("g1_sigma", max=3.0)
    model.set_param_hint("g2_center", value=6730.810, vary=True, min=6726, max=6735)
    model.set_param_hint("g2_sigma", max=3.0)
    return model
...
# SII Guesses
def custom_guess_function(data, x, self=model):
    return tc.models._guess_multiline2_d(
        self=self,
        data=data,
        x=x,
        sigma0=1.2,
...
fit_settings = {
    ...
    #
    # output options
    "output_filename": "eso120_SII_10x10_mc80",  # saved files will begin with this.
    ...
    * sigma_inst,  # in Angstroms. Will set the minimum sigma for gaussian fits.
    ...
    "lines": [L_SII],  # tc.lines.L_OIII5007, tc.lines.L_Hb4861],
    #
    ...
    "parallel": False,  # whether invoke multiprocess
